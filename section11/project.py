...
def compare_score(your_cards, computer_cards, is_drawing):
    print(f"Your Cards are: {your_cards}, your score: {sum(your_cards)}")
    print(f"Computer cards are: {computer_cards}, computer score is {sum(computer_cards)}")
    ...
    elif sum(computer_cards) == 21 and is_drawing == False:
    ...
    while is_running:
        ...
        if play == 'y':
            ...
            while is_drawing == True and is_running == True:
                ...
                ## if statements within a function can affect global variables in python!
                ...
                if draw == 'n':
                    is_drawing = False
                    computer_cards.append(random.choice(cards))
                    compare_score(your_cards, computer_cards, is_drawing)
                else:
                    computer_cards.append(random.choice(cards))
                    your_cards.append(random.choice(cards))
                    compare_score(your_cards, computer_cards, is_drawing)
            ...
            print('Unknown answer. Thanks for playing!')
            # calls blackjack function so that it can re-set the program
            blackjack()
