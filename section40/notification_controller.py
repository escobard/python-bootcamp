...
class NotificationController:
  ...
  def send_email(self, body: str, to_address: str) -> None:
    ...
    Args:
        body (str): The body of the email message to be sent.
    ...
    gmail_connection.sendmail(from_addr=self.smtp_email, to_addrs=to_address, msg=f"Subject:Flight deals!\n\n{body}")
    gmail_connection.close()

  def send_flight_matches_emails(self) -> None:
    """
    Fetches flight matches from the DataModel and sends an email for each match to all user emails.
    """
    for flight_match in self.model.get_flight_matches():
